...
- **기준 완화 설정**: `AppSettings.relaxedForm`(기본 꺼짐)이 켜지면 무릎–발끝 허용폭이 `0.10 → 0.15`로 바뀝니다(`FormThresholds.RELAXED_KNEE_TOE_TOLERANCE`, `PROVISIONAL`). **판정에 영향을 주는 유일한 사용자 설정입니다.** 근거: 실측 240 rep에서 0.10이 정상 의도 세션의 62%에 경고를 냈고 0.15는 45%로 내리면서 모임 검출을 15%까지 유지합니다(0.20은 7%, 0.25는 **0%**라 더 넓히지 않았습니다). 검출력 근거는 기기 시험 한 번뿐입니다. **⚠ `rep_records`에 어느 기준으로 판정했는지가 남지 않습니다** — 나중에 임계값을 재분석할 때 켜짐/꺼짐이 섞인 데이터가 됩니다. 컬럼을 추가할 때 함께 넣으세요
