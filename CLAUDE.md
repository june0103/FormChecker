...
- **준비 자세가 통과해야 기준 자세 3초 측정이 시작됩니다.** 막는 항목은 근거가 확실한 것만입니다(방향·무릎 펴기·발끝 방향·좌우 균형) — **발 간격은 허용폭이 `PROVISIONAL`이라 안내만 하고 막지 않습니다.** 막는 항목이 오탐하면 사용자가 앱을 아예 쓸 수 없습니다(`ReadyIssue.blocking`). 못 본 항목은 통과로 봅니다 — 안 그러면 측면 촬영으로는 영원히 시작할 수 없습니다
...
- **카메라 프리뷰는 이미 최대 크기입니다** — `aspectRatio(3:4)`가 가로에서 먼저 막혀, 아래 컨트롤을 없애도 프리뷰는 커지지 않고 위아래 빈 공간만 늘어납니다. 늘리려면 좌우 여백을 없애 폭을 키워야 합니다(약 8%)
