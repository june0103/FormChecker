...
- **피드백 표시 시간**: `FeedbackHold`가 항목당 하나씩 붙잡습니다. 기본 2초는 *"최저점에서 난 경고를 다 올라온 뒤에 읽을 수 있어야 한다"*(상승 p95 1,171ms)에서 나왔지만, **깊이만 0.9초입니다**(`SHORT_HOLD_CHECKS`). `SHALLOW_DEPTH`는 하강·상승의 얕은 구간에서 각각 발화하는데 2초가 rep 중앙값(1,725ms)보다 길어 **사실상 연속으로 떠 있었고**, 충분히 깊게 앉는 사람도 계속 봤습니다. 화면은 붙잡힌 항목을 **전부** 그리므로(HUD가 `forEach`, 하단 고정이라 위로 쌓임) 막힌 것은 아니지만 시간을 통째로 차지하면 결과가 같습니다. **정렬은 도착 순(새로 뜬 것이 맨 위)입니다** — 우선순위 순이 아닙니다. 새 오류가 자기 순위 자리에 끼어들면 눈에 안 띄고, 하단 고정이라 위에 붙이면 기존 줄이 안 밀립니다. 같은 프레임에 함께 나면 그때만 선언 순서로 가릅니다. **새 판정의 hold를 정할 때 "언제 읽어야 하는 지시인가"를 먼저 물으세요**
