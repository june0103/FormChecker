...
data class FormThresholds(
    ...
    /**
     * 상체가 수직에서 이 각도 이상 기울면 경고한다. `hip→neck` 벡터와 수직축의 각.
     *
     * 스쿼트에서 어느 정도의 전방 숙임은 정상이다 — 고관절을 접으면 상체가 따라 기운다.
     * 그래서 경계는 "기울었는가"가 아니라 "정상 범위를 넘었는가"다.
     *
     * ## 체형을 지우는 데 성공한 특징 — 지금까지 중 최고
     * 측면 3세션 44 rep의 rep 최대값이 사람별 37.1° / 36.6° / 39.1°로 모였다.
     * **사람 간 MAD가 사람 내의 0.44배**로, 골반 쏠림(1.04배)보다도 낮다. 정규화가 아예
     * 필요 없다 — 각도는 그 자체로 스케일 불변이고, 상체 기울기는 체절 비율에 거의
     * 의존하지 않는다. 공통 임계값을 쓸 근거가 확실한 몇 안 되는 항목이다.
     *
     * ## 실측 분포 (측면 3세션 / 3명 / rep 내부 1875프레임)
     *
     * | 국면 | p50 | p95 | p99 | 최대 |
     * |---|---|---|---|---|
     * | 선 자세(READY) | 2.4° | 9.8° | 13.5° | 20.7° |
     * | 하강 | 28.1° | 38.7° | 40.3° | 41.1° |
     * | 최저점 | 36.7° | 39.5° | 40.6° | 41.0° |
     * | 상승 | 23.5° | 37.0° | 38.8° | 40.5° |
     * | **rep 내부 전체** | 29.5° | 38.8° | **40.1°** | **41.1°** |
     *
     * 경고가 프레임 단위로 나가므로 최저점만 보면 안 된다 — 하강 최대(41.1°)가 최저점
     * 최대(41.0°)보다 높다. 위 표는 rep 안의 모든 프레임을 본 것이다.
     *
     * ## 45°를 유지한 이유
     * 정상 표본 최대가 41.1°이므로 45°는 프레임·rep 모두 **오탐 0%**다. 여유 3.9°는
     * 사람 내 MAD(1.24°)의 3.1배다.
     *
     * 0% 오탐을 유지하는 가장 빡빡한 값은 **42°**지만(41.1° 대비 여유 0.9°) 사람 내
     * MAD의 0.7배뿐이다. 표본 3명으로 그렇게 좁히면 네 번째 사람에서 오탐이 난다.
     *
     * ## ⚠ 오류를 잡는지는 검증되지 않았다
     * [CaptureIntent.TRUNK_LEAN] 세션이 아직 없다. 정상 상한(41.1°)은 확인됐지만 **과도한
     * 숙임이 45°를 넘는지는 모른다.** 정상이 41°까지 오는데 경계가 45°면 잡을 수 있는 폭이
     * 4°뿐이라, 실제로는 더 내려야 할 가능성이 있다. 의도적 상체 숙임 세션이 오면 정상
     * 상한과 오류 하한 사이에 다시 놓을 것.
     */
    ...
    /**
     * 발 간격이 이보다 좁으면 무릎 정렬 판정을 보류한다(정규화 좌표).
     *
     * 발목 간격은 정면 특징들의 **분모**다. 두 발목이 화면에서 겹치면 작은 분모가 편차를
     * 통째로 부풀린다 — 실측 P009 세션이 발목 간격 p5 0.0118까지 무너져
     * `kneeToeDeviation` 최대 **7.761**(허용폭 0.10)을 냈다. 정상 11세션의 최소 발목
     * 간격이 0.129~0.238이므로 0.04는 파손 프레임만 걸러낸다 — P009의 불가능한 프레임
     * 50개는 전부, 정상 세션은 0프레임.
     *
     * 값 자체는 아직 `PROVISIONAL`이다. 다만 정상과 파손 사이의 여유가 3배 이상이라
     * 자릿수만 맞으면 되는 구간이다.
     */
