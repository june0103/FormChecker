...
data class FormThresholds(
     ...
     * |---|---|---|---|---|---|---|
     * | P001 | 0.589 | 20 | +0.050 | +0.078 | **+0.089** | 0.0083 |
     ...
     *
     * ## 왜 필요한가
     * 뒤꿈치 들림은 발끝을 지면 기준점으로 쓴다. 발이 짧게 보이면 발끝과 뒤꿈치가 화면에서
     * 가까워져 둘의 높이차가 키포인트 노이즈에 묻힌다.
     *
     * | 사람 | 발/정강이 | 발끝 confidence p50 | rep 최대 |
     * |---|---|---|---|
     * | P001 | 0.589 | 0.716 | +0.089 |
     * | P008 | 0.561 | 0.728 | +0.081 |
     * | **P007** | **0.413** | **0.520** | **+0.144** |
     *
     * 발이 짧게 보인 세션이 정확히 값이 시끄러운 세션이다. 경계는 0.413과 0.561 사이에
     * 두었다.
     *
     * ## ⚠ 나쁜 예가 하나뿐이다 ([ThresholdOrigin.PROVISIONAL])
     * 두 무리 사이 어디에 두는지를 3세션으로는 정할 수 없다. 0.45는 하한(0.413) 쪽에 가까워
     * **관대한 선택**이다 — 중간점(0.487)에 두면 P008(0.561)까지 여유가 0.074로 줄어 정상
     * 세션이 판정 불가가 될 위험이 커진다.
     *
     * 대가는 명확하다: 이 게이트로 **측면 3세션 중 1개(33%)가 뒤꿈치 판정을 받지 못한다.**
     * 그게 틀린 경고를 내는 것보다 낫다는 판단이다.
     *
     * ## 정면의 [ReadyThresholds.minFootLengthRatio]와 다른 값인 이유
     * 분모가 다르다(정면은 다리 길이, 여기는 정강이) — 그리고 애초에 **정면과 측면은 발이
     * 보이는 정도가 다르다.** 정면에서는 발이 깊이축을 향해 크게 단축되므로 정상 범위 자체가
     * 낮다(정면 정상 10세션의 발/정강이 환산 0.25~0.46). 하나의 값으로 합칠 수 없다.
     */
    val minHeelFootRatio: Float = 0.45f,

    /**
     * 무릎이 발끝을 넘은 정도의 한계 `(무릎.x − 발끝.x) / 발 길이`. 측면 전용. **0 — 허용폭이
     * 없다.**
     *
     * ## 허용폭 없이 0을 쓸 수 있는 이유 — 노이즈를 집계가 흡수한다
     * "무릎이 발끝을 넘지 않는다"는 코칭 규칙이 기준점을 0으로 정한다
     * ([ThresholdOrigin.DEFINITION]). 문제는 그 0을 **어떻게 재는가**였다.
     *
     * 실측 측면 2세션(게이트 통과) 915 판정프레임 · 35 rep에서:
     *
     * | 임계값 | 프레임 초과 | rep 경고(한 프레임이라도) | rep 경고(**중앙값**) |
     * |---|---|---|---|
     * | **0.000** | 6% | **46%** | **0%** |
     * | 0.050 | 1% | 11% | 0% |
     * | 0.125 | 0% | 0% | 0% |
     *
     * 프레임 단위로 0을 쓰면 정상 rep의 **46%**가 걸린다(무릎 정렬이 62%였던 것과 같은
     * 증폭 — rep 안 한 프레임이라도 넘으면 경고하는 규칙). 그런데 **rep 중앙값으로 보면
     * 0에서도 오탐이 0%**다.
     *
     * 그래서 허용폭을 0으로 두고 노이즈는 [kneePastToeOf]의 중앙값이 흡수한다. 예전에는
     * 0.125(사람내 MAD 0.0374의 3배)를 썼는데, **기기에서 무릎을 앞으로 밀어도 잡히지
     * 않았다**(사용자 확인, 2026-08-24) — 정상 프레임 최대가 +0.067이라 그 두 배를 넘겨야
     * 했다.
     *
     * ## ⚠ 게이트를 통과한 사람이 둘뿐이다
     * 측면 실측이 3세션·3명인데 그중 하나가 [minHeelFootRatio] 게이트에 걸린다. 아래
     * 발/정강이는 **기준선(READY 프레임)에서 잰 값**이다 — 게이트가 실제로 보는 값이다:
     *
     * | 사람 | 발/정강이 | 게이트 | rep 중앙값 돌출 | 프레임 최대 |
     * |---|---|---|---|---|
     * | P001 | 0.565 | 통과 | −0.050 | +0.067 |
     * | P008 | 0.539 | 통과 | −0.143 | +0.067 |
     * | **P007** | **0.360** | **탈락** | **+0.385** | **+1.857** |
     *
     * **발끝을 넘은 rep은 전부 게이트 탈락 세션에서 나왔다** — P007은 rep 중앙값으로도
     * 9/9가 0을 넘고, 통과한 두 사람은 35/35가 안 넘는다.
     *
     * ## 왜 게이트가 필요한가 — 분모와 분자가 함께 무너진다
     * 발이 짧게 보이면 발끝이 뒤꿈치 쪽으로 당겨진다. 그러면 **분자(무릎 − 발끝)는 커지고
     * 분모(발 길이)는 작아져** 비율이 부풀려진다. P007의 +0.385는 자세가 아니라 그 왜곡으로
     * 보는 것이 맞다. 뒤꿈치 들림이 같은 이유로 같은 게이트를 쓴다.
     *
     * ## 무엇이 미검증인가
     * **"정상을 오탐하지 않는가"만** 확인됐다(통과 2명 35 rep에서 0건). **"실제로 무릎을
     * 앞으로 내밀었을 때 잡는가"는 미검증이다** — 의도적 오류 세션이 없다. 다만 기준점이
     * 0이므로 예전 0.125처럼 "영원히 안 뜨는" 상태는 아니다: 통과 두 사람의 프레임 6%가
     * 이미 0을 넘고 있어 신호가 그 경계 근처에서 살아 있다.
     *
     * **측면 정상 세션과 의도적 `KNEE_PAST_TOE` 세션을 찍은 뒤 재검토할 것.** 그때 볼 것은
     * 이 값이 아니라 **집계 방식**이다 — 중앙값이 너무 둔하면 백분위를 올린다.
     */
    val kneeOverToeLimit: Float = 0f,
     ...
     */
    /**
     * rep 하나가 "무릎이 발끝을 넘었다"인가. **rep 단위 판정이다.**
     *
     * ## 왜 프레임이 아니라 rep인가
     * 프레임 하나라도 [kneeOverToeLimit]을 넘으면 경고하는 방식으로는 임계값 0을 쓸 수
     * 없다 — 실측에서 정상 rep의 46%가 걸렸다. 중앙값으로 보면 같은 0에서 0%다
     * (그 표는 [kneeOverToeLimit] 주석에 있다).
     *
     * 깊이가 이미 같은 구조다 — 프레임 단위 `SHALLOW`는 "지나가는 중"이라 버리고 rep
     * 최저점으로 판정한다. 무릎 위치도 **한 순간 스쳤는가**가 아니라 **그 rep이 그런
     * 모양이었나**를 묻는 것이 맞다.
     *
     * ## 중앙값이다
     * 평균이면 한 프레임의 튐이 그대로 들어온다. 최댓값은 프레임 단위와 같아진다.
     * 중앙값은 rep의 절반 이상이 그 위였는지를 묻는다.
     *
     * @param values 이 rep에서 게이트를 통과한 프레임들의 돌출값. 게이트가 닫혀 있었다면
     *   비어 있다.
     * @return 판정할 값이 없으면 **null**이다 — false가 아니다. "안 넘었다"와 "못 봤다"는
     *   다르고, 리포트가 못 본 항목을 말해야 한다.
     */
    fun kneePastToeOf(values: List<Float>): Boolean? {
        ...
        return median > kneeOverToeLimit
    }
    ...
    companion object {
        /**
         * 사용자가 "기준 완화"를 켰을 때 쓰는 [kneeToeToleranceRatio]. **0.15.**
         *
         * ## 왜 이 값인가 ([ThresholdOrigin.PROVISIONAL])
         * 실측 12세션 240 rep을 프로덕션 공식으로 다시 계산한 rep 경고율이다:
         *
         * | 허용폭 | 사람내 MAD 배수 | 모임 | 벌어짐 | 둘 중 하나 |
         * |---|---|---|---|---|
         * | 0.10 (기본) | 3× | 28% | 34% | **62%** |
         * | **0.15** | **4.5×** | **15%** | **30%** | **45%** |
         * | 0.20 | 6× | 7% | 28% | 34% |
         * | 0.25 | 7.5× | **0%** | 18% | 18% |
         *
         * 정상 의도 세션에서 62%가 경고를 받았고, 0.15는 그것을 45%로 내린다.
         *
         * ## 왜 더 넓히지 않았나
         * **모임 검출이 먼저 사라진다.** 0.20에서 7%, 0.25에서 **0%**다 — 무릎이 안쪽으로
         * 모이는 것은 부상과 연결된 방향인데 그 구간은 완화가 아니라 사실상 끈 것이다.
         * 0.15는 모임을 15%까지 유지한다.
         *
         * ## 검출력 근거는 기기 시험 한 번이다
         * "0.15까지 넓혀도 피드백 검출은 잘한다"는 것을 **개발 화면의 허용폭 전환 버튼으로
         * 기기에서 확인했다**(사용자 확인, 2026-08-24). 그 시험이 이 값의 유일한 검출력
         * 근거이므로 `PROVISIONAL`이다 — 의도적 오류 세션(`CaptureIntent`)이 촬영되면
         * 오탐율과 검출율을 함께 재서 확정할 값이다.
         *
         * ## [ORIGINS]에 넣지 않은 이유
         * 그 맵은 `FormThresholds`의 **프로퍼티** 출처를 적는 곳이고, 이 값은 프로퍼티가
         * 아니라 `kneeToeToleranceRatio`에 대입되는 후보값이다. 실제로 판정에 쓰이는 값의
         * 출처는 여전히 `kneeToeToleranceRatio` 항목이 가리킨다.
         */
        ...
        val ORIGINS: Map<String, ThresholdOrigin> = mapOf(
            ...
            // 임계값이 아니라 `kneePastToeOf`의 rep 중앙값이 흡수한다. 다만 게이트를
            // 통과한 사람이 둘뿐이고 "오류를 잡는가"는 미검증이다.
            "kneeOverToeLimit" to ThresholdOrigin.DEFINITION,
...
/**
 * 촬영 방향 판별 임계값. **[CaptureQuality.estimateView]가 유일한 소비자다.**
 *
 * 어깨 폭을 몸통 길이로 나눈 비율을 쓴다 — 정면에서는 두 어깨가 넓게 벌어지고 측면에서는
 * 앞뒤로 겹쳐 거의 한 점이 된다. 몸통 길이로 정규화하면 체격·카메라 거리와 무관해진다.
 *
 * ## 실측으로 확인했다 (정면 12 / 측면 3세션, 정답 라벨 대조)
 * 세션별 READY 중앙값이 **정면 0.675~0.731 / 측면 0.058~0.105**로, 0.15와 0.55 사이가
 * 비어 있다. 세션 단위 다수결은 15/15 전부 맞는다.
 *
 * 프레임 단위로는 쓸 수 없다 — 정면 0.5% / 측면 0.5%가 반대로 읽히고 그게 두 세션에
 * 몰려 있다([CaptureQuality.estimateView] 주석의 표).
 */
data class AngleDetectionThresholds(
    /**
     * 어깨폭/몸통길이가 이 이상이면 정면으로 본다.
     *
     * 정면 12세션의 최솟값이 READY 0.601, 동작 중 0.577이므로 0.55는 여유가 좁다. 다만
     * 측면 쪽 최댓값이 0.157(발이 잘 안 보인 한 세션만 0.678까지 튄다)이라 위로 올릴 수도
     * 있는데, 그러면 애매 구간이 넓어져 판별 실패가 늘어난다. 다수결로 쓰므로 유지한다.
     */
    val frontRatio: Float = 0.55f,

    /**
     * 이 이하면 측면으로 본다. 두 값 사이 구간은 45도로 보고 판정을 보류한다.
     *
     * ## 0.25에서 0.30으로 올렸다
     * 예전에는 이 파일이 0.25, [CaptureQuality]가 0.30으로 **갈라져 있었다** — 같은 것을
     * 재는 두 벌의 숫자였다. 실측에서 0.30이 낫다: 발이 잘 안 보인 측면 세션의 애매 비율이
     * **13.7% → 9.0%**로 줄고, 정면 12세션에서는 어느 값이든 오분류가 0.5%로 같다.
     */
    val sideRatio: Float = 0.30f,

    /** 몸통이 이보다 짧게 보이면(너무 멀거나 가려짐) 방향을 추정하지 않는다. */
    val minTorsoLength: Float = 0.05f,
) {
    companion object {
        val ORIGINS: Map<String, ThresholdOrigin> = mapOf(
            // 두 경계 모두 정답 라벨이 있는 15세션 분포에서 확인했다.
            "frontRatio" to ThresholdOrigin.DATA_DERIVED,
            "sideRatio" to ThresholdOrigin.DATA_DERIVED,
            // 몸통이 짧게 보이는 세션을 따로 재보지 않았다.
            "minTorsoLength" to ThresholdOrigin.PROVISIONAL,
        )
    }
}
