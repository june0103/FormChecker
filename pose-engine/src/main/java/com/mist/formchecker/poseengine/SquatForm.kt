...
data class SquatForm(
    ...
    /**
     * [kneeToeDeviation]이 어느 쪽 무릎에서 나왔는지. 사람 기준 좌/우(화면 아님).
     *
     * ## 왜 남기는가 — 경고에 쪽을 붙이려면 필요하다
     * "무릎을 벌려주세요"보다 "왼쪽 무릎을 벌려주세요"가 실행 가능한 지시다. 좌우 편차는
     * 이미 둘 다 계산하고 그중 나쁜 쪽을 골라 판정하므로, 그 선택 결과를 버리지 않고
     * 남기기만 하면 된다 — **새 임계값이 필요 없다.**
     *
     * ## 좌우 비대칭 경고를 따로 만들지 않은 이유
     * `|좌 − 우|`로 별도 경고를 만들어봤지만 **정상 229 rep에서 새로 잡는 것이 0개**였다.
     * 어느 후보 경계에서도 비대칭이 걸리는 rep은 이미 [kneeAlignment]가 걸고 있었다
     * (0.15 경계에서 27 rep, 그 전부가 기존 경고와 중복). 두 무릎은 함께 움직이므로 차이가
     * 커지려면 한쪽이 이미 허용폭을 넘어야 한다.
     *
     * 반면 **어느 쪽이 나쁜지는 사람 안에서 일관됐다** — 9세션 중 8개가 90~100%
     * (P011은 두 세션 모두 왼쪽 100%). 그래서 경고를 늘리는 대신 기존 경고를 구체화한다.
     */
