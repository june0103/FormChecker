...
enum class RepState {
    ...
    /**
     * 지금 rep 안인가. **자세 판정을 할 수 있는 구간인가와 같은 뜻이다.**
     *
     * ## 왜 이 질문이 여기 있나
     * 모든 자세 임계값이 **rep 프레임에서 도출됐다** — `hipShiftLimit`은 정상 의도 9명
     * 794프레임의 **최저점** 분포, `kneeToeToleranceRatio`는 11명 253 rep 최저점의 사람내
     * MAD다. 선 자세([STANDING])나 미인식([IDLE]) 프레임에 그 값을 대는 것은 **도출 범위
     * 밖에서 판정하는 것**이고, 무릎이 깊이 게이트 없이는 선 자세에서 초과율 100%였던
     * 것처럼 곧바로 오탐이 된다.
     *
     * 실제로 그 오탐이 나왔다: 화면 쪽에 이 게이트가 없어서 **가만히 서 있어도 "양발로
     * 고르게 일어서세요"가 떴다**(사용자 신고, 2026-08-24). 저장 경로에는 처음부터 같은
     * 게이트가 있었다.
     *
     * ## 두 곳에 흩어져 있던 사실을 하나로 모았다
     * `WorkoutViewModel`이 `REP_PHASES` 집합을 들고 있었고, 같은 파일의 `phaseOf`가
     * `IDLE`·`STANDING`에 null을 돌려주며 **같은 사실을 두 번 적고 있었다.** 상태가 스스로
     * 아는 것이므로 여기로 옮긴다 — 새 상태를 추가할 때 한쪽만 고치는 실수를 막는다.
     */
    val isInRep: Boolean
        get() = when (this) {
            DESCENDING, BOTTOM, ASCENDING -> true
            IDLE, STANDING -> false
        }
}
