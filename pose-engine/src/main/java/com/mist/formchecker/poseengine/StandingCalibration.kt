...
data class StandingCalibration(
    ...
    enum class Problem(val message: String, val blocking: Boolean) {
        ...
        /**
         * 정면도 측면도 아닌 비스듬한 방향. **차단한다.**
         *
         * 촬영 방향을 사용자가 고르지 않고 이 창에서 판별하므로, 판별이 안 되면 어느
         * 판정을 켤지 정할 수 없다. 45도는 정면 기준도 측면 기준도 대응하지 않는다
         * (수집 화면의 `CaptureQuality.Problem.AMBIGUOUS_VIEW`와 같은 이유).
         */
        AMBIGUOUS_VIEW("카메라를 똑바로 마주 보거나 옆으로 서 주세요 (지금은 비스듬해요)", true),
