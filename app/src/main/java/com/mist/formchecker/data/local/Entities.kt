...
@Entity(
     ...
     *
     * 실측(`DbQueryBenchmark`, Galaxy S24 Ultra, **p95**): 세션 2,000개에서
     * **0.20ms → 0.10ms**. **효과가 세션 수에 비례해 커진다** — 500개에서는 거의
     * 차이가 없고(1.1배) 2,000개에서 2.0배다. 지금 크기에서 급한 문제였다기보다 **커지는 것을 미리 막는 쪽**이다.
     *
     * ## 기록 목록은 이 인덱스로 빨라지지 않는다
     * `ORDER BY started_at DESC`가 있어 도움이 될 것 같지만, 실측에서 세션 2,000개일 때
     * 오히려 **조금 느렸다**(p95 19.3ms → 20.9ms). 보조 인덱스로 훑으면 행마다 본체를 따로
